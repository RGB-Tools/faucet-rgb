...
def test_control_delete(get_app):
    ...
    transfers_failed = [t for t in asset_transfers if t.status == rgb_lib.TransferStatus.FAILED]
...
def test_control_fail(get_app):
    ...
    transfers_wait_counterparty = [
        t for t in asset_transfers if t.status == rgb_lib.TransferStatus.WAITING_COUNTERPARTY
    ]
...
def test_control_requests(get_app):  # pylint: disable=too-many-statements
    ...
    assert resp.status_code == 200
    assert len(resp.json["requests"]) == 2
    assert all(r.get("asset_group") == asset_group for r in resp.json["requests"])
    ...
    assert resp.status_code == 200
    assert len(resp.json["requests"]) == 1
    assert all(r.get("asset_id") == req.asset_id for r in resp.json["requests"])
    ...
    assert resp.status_code == 200
    assert len(resp.json["requests"]) == 1
    assert all(r.get("recipient_id") == req.recipient_id for r in resp.json["requests"])
    assert all(r.get("amount") == req.amount for r in resp.json["requests"])
    ...
    assert resp.status_code == 200
    assert len(resp.json["requests"]) == 1
    assert all(r.get("wallet_id") == req.wallet_id for r in resp.json["requests"])
    assert all(r.get("amount") == req.amount for r in resp.json["requests"])
...
def test_reserve_topuprgb(get_app):  # pylint: disable=too-many-locals
    ...
    wait_sched_process_pending(app)
    wait_xfer_status(user["wallet"], user["online"], asset_id, 1, "WAITING_CONFIRMATIONS")
    ...
    wait_xfer_status(wallet, app.config["ONLINE"], asset_id, 4, "WAITING_CONFIRMATIONS")
